...
class ROIFuser(nn.Module):
    ...
    def roi_self_attention_learned(self, roi_hidden_states, instance_masks, position_feat):
        batch_size, num_roi, channel, roi_size_h, roi_size_w = roi_hidden_states.shape            
        ...
        roi_hidden_states = rearrange(roi_hidden_states, "b n c h w -> (b n) (h w) c")
        instance_masks = rearrange(instance_masks, "b n -> (b n)").bool()
        position_feat = rearrange(position_feat, "b n c -> (b n) 1 c")

        n_visual = roi_hidden_states.shape[1]
        roi_attn_output = torch.zeros_like(roi_hidden_states)

        roi_hidden_states = self.norm1(torch.cat([roi_hidden_states, position_feat], dim=1))

        roi_attn_output[instance_masks] = self.roi_self_attn(roi_hidden_states[instance_masks])[:, :n_visual, :].to(dtype=roi_attn_output.dtype)
        roi_attn_output = rearrange(roi_attn_output, "(b n) (h w) c -> b n c h w", b=batch_size, h=roi_size_h)
    ...
    def forward(self, normed_hidden_states, hidden_states, attn_output, pretrained_attn, instance_boxes, instance_masks, instance_embeddings, instance_pooled_embeddings, spatial_size, position_feat, global_bg_mask):
        ...
        roi_attn_output = roi_attn_output + self.alpha_roi_self_attn.tanh() * self.roi_self_attention_learned(roi_attn_output, instance_masks, position_feat)
        ...
        hidden_states = hidden_states + self.alpha_dense.tanh() * self.ff(self.norm2(hidden_states))
